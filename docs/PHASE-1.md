...
- `data/overview.ts` owns approved facts and four topic summaries. Active app components must not import draft sessions, speakers, registration configuration, or editorial data.
...
- `/disaster-law-symposium-2026.ics` is generated at build time from overview content. DTSTART is DATE 20261029; DTEND is exclusive DATE 20261030. It is an all-day placeholder, not a timing announcement.
...
`archive/phase-0/` holds the previous secondary and staging route entrypoints, the pre-change stylesheet (including local staging work), previous homepage/layout/header/footer, and unpublished public assets. `data/sessions.ts`, `data/speakers.ts`, and other draft content and unused components remain local source only. The stakeholder PDF remains at `artifacts/disaster-law-symposium-2026-staging.pdf`. None of these locations are copied into the static export. Archive TypeScript is excluded from the active typecheck; restore and review it before reuse.
